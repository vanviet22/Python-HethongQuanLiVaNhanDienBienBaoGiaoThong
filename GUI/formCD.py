...
class Ui_MainWindow(object):
    ...
    def setupUi(self, MainWindow):
        ...
        self.widgetTieuDe.setStyleSheet("background-color: #ADD8E6;")
        ...
        self.label.setStyleSheet("QLabel#label { color: #008000; }")
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.label.setText(_translate("MainWindow", "Quản lí biển báo chỉ dẫn "))
    ...
    def hienThi(self):
        try:
            ...
            query = "SELECT * FROM Bienbaochidan"
            cursor = conn.cursor()
    ...
    def them(self):
        if(self.textEdit.toPlainText()==""):
            ...
            try:
                conn=mysql.connector.connect(user='root', password='123456789', host='localhost', database='doan_py')
                cursor = conn.cursor()
                sql="INSERT INTO bienbaochidan(Ten_BienBaoChiDan,Mieuta,HinhAnhBBCD) VALUES (%s,%s,%s)"
        ...
        if (selected_row >= 0 and selected_row < len(self.dsBB)):
            ...
            if self.textEdit.toPlainText() == "":
                ...
                try:
                    conn = mysql.connector.connect(user='root', password='123456789', host='localhost',database='doan_py')
                    cursor = conn.cursor()
                    sql="UPDATE bienbaochidan SET Ten_BienBaoChiDan= %s , Mieuta= %s , HinhAnhBBCD= %s WHERE Ten_BienBaoChiDan= %s"
    ...
    def xoa(self):
         ...
         if (selected_row >= 0 and selected_row < len(self.dsBB)):
            ...
            try:
                conn = mysql.connector.connect(user='root', password='123456789', host='localhost', database='doan_py')
                cursor = conn.cursor()
                sql="DELETE FROM bienbaochidan WHERE Ten_BienBaoChiDan = %s"
